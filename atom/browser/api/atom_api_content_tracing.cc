...
v8::Local<v8::Promise> StopRecording(v8::Isolate* isolate,
  ...
  TracingController::GetInstance()->StopTracing(GetTraceDataEndpoint(
      path,
      base::Bind(atom::util::CopyablePromise::ResolveCopyablePromise<base::FilePath>,
                 atom::util::CopyablePromise(promise), std::move(path)));
  return handle;
}
...
v8::Local<v8::Promise> StartTracing(
    v8::Isolate* isolate const base::trace_event::TraceConfig& trace_config) {
  atom::util::Promise promise(isolate);
  v8::Local<v8::Promise> handle = promise.GetHandle();
